 ...
 */
public class GameTreeView extends JPanel {
    ...
    public static class FillColorAction extends ColorAction {
        private ColorMap cmap = new ColorMap(
            ColorLib.getInterpolatedPalette(10,
                ColorLib.rgb(200,200,255), ColorLib.rgb(85,85,85)), 0, 9);

        public FillColorAction(String group) {
            super(group, VisualItem.FILLCOLOR);
        }
        public FillColorAction(String group, String s) {
            super(group, s);
        }
...
//                } else {
                    if ( item.getBoolean(CHOICE) )
                        return ColorLib.rgb(255,0,0);
                    else
                        return cmap.getColor(nitem.getDepth());
//                }
            } else if(item instanceof EdgeItem) {
                EdgeItem eitem = (EdgeItem)item;
                  if ( eitem.getTargetItem().getBoolean(CHOICE) && eitem.getSourceItem().getBoolean(CHOICE))
                      return ColorLib.rgb(255,0,0);
                  else
    ...
    public GameTreeView(Graph g, String label) {
        ...
        ColorAction fill = new FillColorAction(nodes);
        ColorAction fillEdges = new FillColorAction(edges, VisualItem.STROKECOLOR);

        //new ColorAction(nodes,  VisualItem.FILLCOLOR, ColorLib.rgb(200,200,255));
        ...
        //draw.add(new ColorAction(edges, VisualItem.FILLCOLOR, 0));
        //draw.add(new ColorAction(edges, VisualItem.STROKECOLOR, ColorLib.gray(200)));
        
        ActionList animate = new ActionList(Activity.INFINITY);
        animate.add(new ForceDirectedLayout(graph));
        animate.add(fill);
        animate.add(fillEdges);
...
//        add(split);
    ...
    public static JFrame demo(String datafile, String label) {
        ...
        g.getNodeTable().addColumns(CHOICE_SCHEMA);

        Node n1 = g.addNode();
        n1.setString(LABEL, "hi");
        Node n2 = g.addNode();
        n2.setString(LABEL, "guy");
        g.addEdge(n1, n2);
//        if ( datafile == null ) {
...
//        }
    ...
    public static JFrame draw(Graph g){
        UILib.setPlatformLookAndFeel();
    ...
    public static JFrame demo(Graph g, String label) {
        final GameTreeView view = new GameTreeView(g, label);
...
//        frame.setJMenuBar(menubar);
    ...
    public abstract static class GraphMenuAction extends AbstractAction {
        private GameTreeView m_view;
        public GraphMenuAction(String name, String accel, GameTreeView view) {
            m_view = view;
    ...
    public static class OpenGraphAction extends AbstractAction {
        ...
        public OpenGraphAction(GameTreeView view) {
            m_view = view;
    ...
    public static final Schema CHOICE_SCHEMA = new Schema();
    static {
        LABEL_SCHEMA.addColumn(LABEL, String.class, "");
        CHOICE_SCHEMA.addColumn(CHOICE, boolean.class, false);
    }
    
} // end of class GameTreeView
